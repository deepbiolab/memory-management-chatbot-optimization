...
2. The **copy assignment operator**: The **default assignment operation performs a member-wise shallow copy**, which **does not copy the content behind the resource handle**. If a deep copy is needed, it has be implemented by the programmer.
...
In the previous section, we have taken a look at the **three smart pointer types** in C++. In addition to smart pointers, you are now **also familiar with move semantics**, which is of particular importance in this section. In this section, we explore how the **ownership of smart pointers can be transferred to another scope**. Smart pointers follow the **rule of five**, implementing **move constructors** and **move assignment operators** alongside **copy constructors**, **copy assignment operators**, and **destructors**, allowing them to be moved efficiently by simply transferring their internal handle. While **`std::shared_ptr` has slightly higher overhead** due to reference counting, **this is negligible unless in high-performance applications**. Additionally, we will review C++ guidelines on **when** to **pass smart pointers by value** or **reference** and examine **best practices for returning smart pointers from functions.**

In modern C++, there are various ways of doing this and in many cases, the method of choice has an impact on both performance and code robustness. The basis of this section are the C++ core guidelines on smart pointers, some of which we will be examining in the following.
...
int main() {
  ...
  if (uniquePtr) uniquePtr->printVal();
...
Consider the example on the below. 
...
int main() {
  ...
  std::cout << "shared_ptr (ref_cnt= " << sharedPtr.use_count() << ") "
            << &sharedPtr;
  sharedPtr->printVal();
...
void modifyUniquePtr(std::unique_ptr<int>& ptr) {
    ptr.reset(new int(42));  // reseat
}


void modifySharedPtr(std::shared_ptr<int>& ptr) {
    ptr = std::make_shared<int>(99);  // reseat
}

int main() {
    // unique_ptr as non-const reference into function
    std::unique_ptr<int> uniquePtr = std::make_unique<int>(10);
    std::cout << "Before modifyUniquePtr: " << *uniquePtr << std::endl;
    modifyUniquePtr(uniquePtr);
    std::cout << "After modifyUniquePtr: " << *uniquePtr << std::endl;

    // shared_ptr as non-const reference into function
    std::shared_ptr<int> sharedPtr = std::make_shared<int>(20);
    std::cout << "Before modifySharedPtr: " << *sharedPtr << std::endl;
    modifySharedPtr(sharedPtr);
    std::cout << "After modifySharedPtr: " << *sharedPtr << std::endl;
...
```bash
Before modifyUniquePtr: 10
After modifyUniquePtr: 42
Before modifySharedPtr: 20
After modifySharedPtr: 99
...
// read-only unique_ptr managed object
void readUniquePtr(const std::unique_ptr<int>& ptr) {
    std::cout << "Reading unique_ptr value: " << *ptr << std::endl;
}

// read-only shared_ptr managed object
void readSharedPtr(const std::shared_ptr<int>& ptr) {
    std::cout << "Reading shared_ptr value: " << *ptr << std::endl;
}

int main() {
    // unique_ptr
    std::unique_ptr<int> uniquePtr = std::make_unique<int>(30);
    readUniquePtr(uniquePtr);

    // shared_ptr
    std::shared_ptr<int> sharedPtr = std::make_shared<int>(40);
    readSharedPtr(sharedPtr);
...
```bash
Reading unique_ptr value: 30
Reading shared_ptr value: 40
...
// passing raw pointer of smart pointer
void inspectRawPointer(const int* ptr) {
    if (ptr) {
        std::cout << "Inspecting raw pointer value: " << *ptr << std::endl;
    } else {
        std::cout << "Pointer is null." << std::endl;
    }
}

// passing managed object reference of smart pointer
void inspectReference(const int& value) {
    std::cout << "Inspecting reference value: " << value << std::endl;
}

int main() {
    // unique_ptr
    std::unique_ptr<int> uniquePtr = std::make_unique<int>(50);
    inspectRawPointer(uniquePtr.get());  
    inspectReference(*uniquePtr);     

    // shared_ptr
    std::shared_ptr<int> sharedPtr = std::make_shared<int>(60);
    inspectRawPointer(sharedPtr.get());
    inspectReference(*sharedPtr); 
...
With return values, the same logic that we have used for passing smart pointers to functions applies: **Return a smart pointer, both unique or shared, if the caller needs to manipulate or access the pointer properties**. 
...
// return unique_ptr managed object's raw pointer
int* getRawPointerFromUniquePtr(const std::unique_ptr<int>& ptr) {
    return ptr.get();  // use get() acquire managed object's raw pointer
}

// return shared_ptr managed object's raw pointer
int* getRawPointerFromSharedPtr(const std::shared_ptr<int>& ptr) {
    return ptr.get();  // use get() acquire managed object's raw pointer
}
...
int main() {
    ...
    int* rawPtrFromUnique = getRawPointerFromUniquePtr(uniquePtr);
    std::cout << "Raw pointer from unique_ptr: " << *rawPtrFromUnique << std::endl;

    // shared_ptr
    std::shared_ptr<int> sharedPtr = std::make_shared<int>(99);
    int* rawPtrFromShared = getRawPointerFromSharedPtr(sharedPtr);
    std::cout << "Raw pointer from shared_ptr: " << *rawPtrFromShared << std::endl;
...
Using this call structure, the function states that it might modify the smart pointer, e.g. by redirecting it to another object. I**t is not recommended to use** it for accepting an object **only because we should avoid restricting ourselves unnecessarily to a particular object lifetime strategy on the caller side.**
...
// 3. share ownership
void share_ownership(std::shared_ptr<Object> obj) {

}

int main() {
    // unique_ptr
    auto unique = std::make_unique<Object>();
    observe(unique.get());      // 传递原始指针
    manipulate(*unique);        // 通过解引用传递引用
    take_ownership(std::move(unique));  // 转移所有权

    // shared_ptr
    auto shared = std::make_shared<Object>();
    observe(shared.get());      // 传递原始指针
    manipulate(*shared);        // 通过解引用传递引用
    share_ownership(shared);    // 共享所有权
...
This course on memory management covered a wide range of topics, starting with memory types, the process memory model, and automatic memory allocation on the stack, along with call by value and call by reference. It delved into dynamic memory allocation on the heap using functions like `malloc` and `free`, as well as object-oriented counterparts `new` and `delete`, leading to lessons on smart pointers. Key concepts such as copy semantics, L-values, R-values, move semantics, and resource acquisition is initialization (RAII) were explored, highlighting their role in resource management and smart pointers. The course also covered the three types of smart pointers, when to use them, and when raw pointers are appropriate, as well as methods for passing smart pointers into functions. 
